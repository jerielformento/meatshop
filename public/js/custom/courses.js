...
function ajxDispList( page, data, appendTo, startPage, endPage, currPage ) { 
	...
	$.post(page, data, function(response) {
		...
		var count = Object.keys(json["courselist"]).length;
		console.log(json);
		var returnString = "";
		
		returnString = "<thead>";
		returnString += "<tr>";
		returnString += "<th class=\"active\"><span>Course ID</span></th>";	
		returnString += "<th class=\"active\"><span>Course Type</span></th>";	
		returnString += "<th class=\"active\"><span>Time</span></th>";	
		returnString += "<th class=\"active\"><span>Date Range</span></th>";	
		returnString += "<th class=\"active\"><span>Action</span></th>";	
		returnString += "</tr>" +
		"</thead>" +
		"<tbody>";

		json['courselist'].forEach(function(ul) {
			returnString += "<tr>";
			returnString += "<td>" + ul['course_id'] + "</td>";
			returnString += "<td>" + ul['type'] + "</td>";
			returnString += "<td>" + ul['time'] + "</td>";
			returnString += "<td>" + ul['date_range'] + "</td>";
			//returnString += "<td>" + ul['date_created'] + "</td>";
			//returnString += "<td>" + ul['date_created'] + "</td>";
			returnString += "<td style=\"text-align:center;\">";
            returnString += "<a class=\"btn btn-primary btn-sm mr-2 u-view\" href=\"courses/view?id=" + ul['id'] + "\" title=\"View\"><span class=\"fa fa-fw fa-eye\" style=\"cursor:pointer;\"></span></a>";
            returnString += "<a class=\"btn btn-warning btn-sm mr-2 u-edit\" href=\"courses/edit?id=" + ul['id'] + "\" title=\"Edit\"><span class=\"fa fa-fw fa-edit\" style=\"cursor:pointer;\"></span></a>";
...
function managePages( val ) {
	...
	ajxDispList('fetchCourseList', { start: start, end: end }, 'course-list-grid', start, end, opt);
}
            ...
            if(pass === pass_conf) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(json['message'][0] === 'success') {
                            ajxDispList('fetchCourseList', { start: 0, end: 15 }, 'course-list-grid', 0, 15, 1);
...
$('#sub-sr').on('click', function() {
	...
	ajxDispList('fetchCourseList', { key: search, start: 0, end: 15 }, 'course-list-grid', 0, 15, 1);
});
