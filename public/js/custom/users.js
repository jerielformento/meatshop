...
function ajxDispList( page, data, appendTo, startPage, endPage, currPage ) { 
	...
	$.post(page, data, function(response) {
		...
		var count = Object.keys(json["userlist"]).length;
		console.log(json);
		var returnString = "";
		
		returnString = "<thead>";
		returnString += "<tr>";
		returnString += "<th class=\"active\"><span>Username</span></th>";	
		returnString += "<th class=\"active\"><span>FirstName</span></th>";	
		returnString += "<th class=\"active\"><span>MiddleName</span></th>";	
		returnString += "<th class=\"active\"><span>LastName</span></th>";	
		returnString += "<th class=\"active\"><span>Email</span></th>";	
		returnString += "<th class=\"active\"><span>Privilege</span></th>";	
		returnString += "<th class=\"active\"><span>DateAdded</span></th>";	
		//returnString += "<th class=\"active\"><span>Date&nbsp;Created</span></th>";
		returnString += "<th class=\"active\"><span>Action</span></th>";
		returnString += "</tr>" +
		"</thead>" +
		"<tbody>";

		json['userlist'].forEach(function(ul) {
			returnString += "<tr>";
			returnString += "<td>" + ul['username'] + "</td>";
			returnString += "<td>" + ul['first_name'] + "</td>";
			returnString += "<td>" + ul['middle_name'] + "</td>";
			returnString += "<td>" + ul['last_name'] + "</td>";
			returnString += "<td>" + isNull(ul['email_address']) + "</td>";
			returnString += "<td>" + ul['privilege'] + "</td>";
			returnString += "<td>" + ul['date_created'] + "</td>";
			//returnString += "<td>" + ul['date_created'] + "</td>";
			returnString += "<td style=\"text-align:center;\">";
            returnString += "<a class=\"btn btn-warning btn-sm mr-2 u-edit\" href=\"users/edit?id=" + ul['user_id'] + "\" title=\"Edit\"><span class=\"fa fa-fw fa-edit\" title=\"Remove\" style=\"cursor:pointer;\"></span> Edit</a>";
            returnString += "<button class=\"btn btn-danger btn-sm mr-2 u-del\" usnid=\"" + ul['user_id'] + "\" usn=\"" + ul['username'] + "\" title=\"Remove User\"><span class=\"fa fa-fw fa-trash\" title=\"Remove\" style=\"cursor:pointer;\"></span> Delete</button>";
            returnString += "</td>";
			returnString += "</tr>";
		});
...
function managePages( val ) {
	...
	ajxDispList('fetchUserList', { start: start, end: end }, 'user-list-grid', start, end, opt);
}
...
function confBoxEvents() {
            ...
            if(pass === pass_conf) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(json['message'][0] === 'success') {
                            ajxDispList('fetchUserList', { start: 0, end: 15 }, 'user-list-grid', 0, 15, 1);
	...
	$('#user-del').unbind().on('click', function() {
		if(rHandler != "") {
			var elemInfo = startLoader($(this));
...
function listAction() {
	...
	$('td button.u-del').unbind().on('click', function() {
		rHandler = isEmpty($(this).attr('usnid'));

		if(rHandler != "") {
			confirmBox('del-user','Remove',"Do you want to remove <span class='text-danger'>" + $(this).attr('usn') + "</span> user from the list?",'user-del','Remove','danger','show-box');
		}
	});
}
...
$('#sub-sr').on('click', function() {
	...
	ajxDispList('fetchUserList', { key: search, start: 0, end: 15 }, 'user-list-grid', 0, 15, 1);
});
